...
import '../bloc/pending_escrow_bloc.dart';
import '../bloc/pending_escrow_event.dart';
import '../bloc/pending_escrow_state.dart';

class PendingEscrowPage extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (BuildContext context) => PendingEscrowBloc()..add(InitEvent()),
      child: BlocBuilder<PendingEscrowBloc,PendingEscrowState>(builder: (context,state) => _buildPage(context,state)),
    );
  }

  Widget _buildPage(BuildContext context, PendingEscrowState state) {
    final bloc = BlocProvider.of<PendingEscrowBloc>(context);
    return Scaffold(
      appBar: DefaultAppbar(
        title: appLanguage(context).pending_escrows
      ),
      body: SwipeRefresh(
        ...
        ListView.builder(
            ...
            itemBuilder: (ctx,index){
              PendingData data = state.transfers!.response!.escrows!.data![index];
              return Card(
