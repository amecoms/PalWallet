...
import '../bloc/support_details_bloc.dart';
import '../bloc/support_details_event.dart';
import '../bloc/support_details_state.dart';
import 'package:path/path.dart';

import '../data/support_messages.dart';

class SupportDetailsPage extends StatelessWidget {
  SupportDetailsBloc? bloc;
  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (BuildContext context) => SupportDetailsBloc()..add(InitEvent(context)),
      child: BlocBuilder<SupportDetailsBloc,SupportDetailsState>(
        builder: (context,state) => _buildPage(context,state)
      ),
    );
  }

  Widget _buildPage(BuildContext context, SupportDetailsState state) {
    bloc ??= BlocProvider.of<SupportDetailsBloc>(context);

    return Scaffold(
      appBar: DefaultAppbar(
        title: state.messages!=null ? '${appLanguage(context).support_ticket}: ${state.channelId ?? ''}' : appLanguage().messages,
        centerTitle: false
      ),
      body: Stack(
        children: [
  ...
  Widget messageView(SupportMessages messages) {
