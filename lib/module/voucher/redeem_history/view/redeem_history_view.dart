...
import '../bloc/redeem_history_bloc.dart';
import '../bloc/redeem_history_event.dart';
import '../bloc/redeem_history_state.dart';

class RedeemHistoryPage extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (BuildContext context) => RedeemHistoryBloc()..add(InitEvent()),
      child: BlocBuilder<RedeemHistoryBloc,RedeemHistoryState>(builder: (context,state) => _buildPage(context,state)),
    );
  }

  Widget _buildPage(BuildContext context, RedeemHistoryState state) {
    final bloc = BlocProvider.of<RedeemHistoryBloc>(context);
    return Scaffold(
      appBar: DefaultAppbar(
        title: appLanguage(context).redeemed_history
      ),
      body: SwipeRefresh(
        ...
        child: state.transfers == null || state.pageState != PageState.Success ? CircularProgress(height: 1.sh) : state.transfers!.response!.history!.data!.isNotEmpty ?
        ListView.builder(
            itemCount: state.transfers!.response!.history!.data!.length,
            shrinkWrap: true,
            padding: REdgeInsets.symmetric(horizontal: 16),
            itemBuilder: (ctx,index){
              RedeemData data = state.transfers!.response!.history!.data![index];
              return Card(
